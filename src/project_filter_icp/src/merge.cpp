...
public:  
    ...
    {  
        ...
        Eigen::Vector3d rvec(-0.0251048, 0.064995, -0.513204);// No.6  -0.0459966, -0.128626, 0.0558593
        Eigen::Matrix3d rot = Eigen::Matrix3d::Identity();
        double rot_angle = rvec.norm();
	      if (std::fabs(rot_angle) > 1e-6) {
	      	Eigen::Vector3d axis = rvec.normalized();
          rot = Eigen::AngleAxis<double>(rot_angle, axis);
	      }
          
        Eigen::Vector3d tvec(-0.63735, -1.14282, -0.0275067);//No.6  -0.066045, -1.23302, 0.0377227
...
private:  
    ros::NodeHandle nh;   
    ros::Publisher _pub = nh.advertise<sensor_msgs::PointCloud2>("/chatter_pub", 1);;
    ...
    Eigen::Matrix4d _pairTransform = Eigen::Matrix4d::Identity (); 
    //lidar_camera
